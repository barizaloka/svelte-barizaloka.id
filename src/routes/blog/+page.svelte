...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between my-8">
		<div class="flex flex-wrap gap-2">
			{#each categories as cat}
				<button
					...
					class="rounded-full px-4 py-1.5 text-xs font-semibold transition-all {selectedCategory === cat ? 'bg-emerald-500 text-slate-950 shadow-md shadow-emerald-500/20' : 'bg-slate-900 border border-slate-800 text-slate-300 hover:bg-slate-800'}"
				>
					{cat}
				</button>
			{/each}
		...
		<div class="relative w-full sm:w-72">
			...
			<input
				...
				class="w-full rounded-full border border-slate-800 bg-slate-900 py-2.5 pl-10 pr-4 text-xs text-slate-200 placeholder-slate-500 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
			/>
		</div>
	</div>
	...
	{#if filteredPosts.length === 0}
		<div class="rounded-3xl border border-dashed border-slate-800 p-12 text-center text-slate-400 text-sm my-12">
	...
	{:else}
		<div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 my-12">
			{#each filteredPosts as post (post.id)}
				<a
					href="/blog/{post.slug}"
					class="group flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/70 shadow-xl backdrop-blur-md hover:border-emerald-500/40 transition-all duration-300"
				>
					<div class="space-y-4">
						...
						<div class="p-6 space-y-3">
							<div class="flex items-center gap-3 text-[11px] text-slate-400">
								<span class="flex items-center gap-1"><Calendar class="h-3 w-3 text-emerald-400" /> {post.publishedAt}</span>
								<span>•</span>
					...
					<div class="p-6 pt-0 flex items-center justify-between border-t border-slate-800/60 mt-4">
						<div class="flex items-center gap-2">
							<img src={post.author.avatar} alt={post.author.name} class="h-6 w-6 rounded-full object-cover" />
							<span class="text-xs text-slate-300 font-medium">{post.author.name}</span>
						</div>
						<div class="text-xs font-bold text-emerald-400 group-hover:translate-x-1 transition-transform flex items-center gap-1">
							<span>Baca</span>

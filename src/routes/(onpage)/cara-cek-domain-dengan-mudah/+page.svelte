...
<svelte:head>
	...
	/>
	<meta name="keywords" content="cara cek domain, cek ketersediaan domain, cek domain murah, cek whois domain, domain .id, domain .com, daftar domain barizaloka" />
	<meta property="og:title" content="Cara Cek Domain dengan Mudah & Cepat | Barizaloka" />
	<meta property="og:description" content="Ketahui langkah mudah cek ketersediaan domain website Anda, bandingkan harga ekstensi .COM, .ID, .MY.ID, serta amankan nama brand Anda sekarang." />
...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
	...
	<!-- HERO SECTION -->
	<section class="relative my-8 overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950 p-8 sm:p-12 text-white shadow-2xl">
		<div class="pointer-events-none absolute -right-20 -top-20 h-96 w-96 rounded-full bg-emerald-500/20 blur-3xl"></div>

		<div class="relative z-10 max-w-3xl space-y-6">
			<div class="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/20 px-4 py-1.5 text-xs font-bold text-emerald-300">
			...
			<p class="text-base sm:text-lg text-slate-300 leading-relaxed">
				Ingin membuat website tetapi belum tahu apakah nama domain impian Anda masih tersedia? Gunakan simulator cek ketersediaan domain di bawah ini dan pelajari langkah mudah memilih nama domain yang tepat untuk bisnis Anda.
			</p>
		</div>

		<!-- DOMAIN CHECKER WIDGET -->
		<div class="relative z-10 mt-10 rounded-2xl border border-white/10 bg-white/10 p-4 sm:p-6 backdrop-blur-md">
			<form onsubmit={handleCheckDomain} class="flex flex-col sm:flex-row gap-3">
				<div class="relative flex-1">
					...
					<input
						...
						class="w-full rounded-xl border border-slate-700 bg-slate-900/90 pl-11 pr-4 py-3.5 text-sm font-medium text-white placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
					/>
				</div>
				<button
					type="submit"
					disabled={isSearching || !searchQuery.trim()}
					class="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-400 px-7 py-3.5 text-sm font-bold text-slate-950 shadow-lg shadow-emerald-500/25 hover:from-emerald-400 hover:to-teal-300 transition-all disabled:opacity-50"
				>
					{#if isSearching}
						<div class="h-4 w-4 animate-spin rounded-full border-2 border-slate-950 border-t-transparent"></div>
						<span>Memeriksa...</span>
					{:else}
			...
			{#if !searchPerformed && !isSearching}
				<div class="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-300">
					...
					<button type="button" onclick={() => { searchQuery = 'tokoanda'; handleCheckDomain(); }} class="rounded-lg bg-white/10 px-2.5 py-1 hover:bg-white/20 transition-colors">tokoanda</button>
					<button type="button" onclick={() => { searchQuery = 'pesantrenmodern'; handleCheckDomain(); }} class="rounded-lg bg-white/10 px-2.5 py-1 hover:bg-white/20 transition-colors">pesantrenmodern</button>
					<button type="button" onclick={() => { searchQuery = 'jasadesain'; handleCheckDomain(); }} class="rounded-lg bg-white/10 px-2.5 py-1 hover:bg-white/20 transition-colors">jasadesain</button>
				</div>
			{/if}
		</div>
	</section>
		...
		<section class="my-10 space-y-6">
			...
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
				{#each results as item}
					<div
						class={`rounded-2xl border p-5 transition-all shadow-sm flex flex-col justify-between ${
							item.status === 'available'
								? 'border-emerald-500/40 bg-emerald-50/50 dark:bg-emerald-950/20 dark:border-emerald-800'
								: 'border-slate-200 bg-slate-50/80 dark:border-slate-800 dark:bg-slate-900/40 opacity-80'
						}`}
					>
						<div class="space-y-3">
							<div class="flex items-start justify-between">
								<span class="text-lg font-black text-slate-900 dark:text-white">{item.fullName}</span>
								{#if item.status === 'available'}
									<span class="inline-flex items-center gap-1 rounded-full bg-emerald-500/20 px-2.5 py-0.5 text-xs font-bold text-emerald-700 dark:text-emerald-400">
										<CheckCircle2 class="h-3.5 w-3.5" /> Tersedia
									</span>
								{:else}
									<span class="inline-flex items-center gap-1 rounded-full bg-slate-200 dark:bg-slate-800 px-2.5 py-0.5 text-xs font-bold text-slate-600 dark:text-slate-400">
							...
							{#if item.badge}
								<div class="text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 rounded-md px-2 py-0.5 inline-block">
									{item.badge}
								</div>
							{/if}
						...
						<div class="mt-6 border-t border-slate-200 dark:border-slate-800 pt-4 space-y-3">
							{#if item.status === 'available'}
								<div class="flex items-center gap-2">
									<a
										...
										class="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white shadow hover:bg-emerald-500 transition-colors"
									>
									...
									<button
										...
										class="rounded-xl border border-slate-300 dark:border-slate-700 p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
							...
							{:else}
								<div class="text-[11px] font-medium text-slate-500 dark:text-slate-400 italic">
			...
			<div class="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-xs text-emerald-800 dark:text-emerald-300 flex items-center justify-between flex-wrap gap-4">
				<div class="flex items-center gap-2">
					<Sparkles class="h-5 w-5 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
					<span><strong>Butuh website sekalian domain & hosting?</strong> Barizaloka menyediakan paket lengkap siap pakai mulai Rp350rb/tahun!</span>
				</div>
				<a
					href="/harga"
					class="inline-flex items-center gap-1 font-bold text-emerald-700 dark:text-emerald-300 hover:underline"
				>
	...
	<article class="my-16 space-y-16">
		...
		<div class="space-y-6 max-w-4xl">
			...
			<p class="text-sm sm:text-base text-slate-700 dark:text-slate-300 leading-relaxed">
				<strong>Domain</strong> adalah alamat unik di internet yang digunakan pengunjung untuk mengakses website Anda (contoh: <code class="rounded bg-slate-100 dark:bg-slate-800 px-2 py-0.5 font-mono text-emerald-600 dark:text-emerald-400">barizaloka.id</code>). Tanpa domain, pengunjung harus mengetik deretan angka alamat IP server yang rumit.
			</p>

			<p class="text-sm sm:text-base text-slate-700 dark:text-slate-300 leading-relaxed">
				Sistem nama domain bekerja secara universal dengan prinsip <em>"First Come, First Served"</em> (siapa cepat, dia dapat). Artinya, tidak ada dua pihak yang dapat menguasai nama domain yang persis sama dengan ekstensi yang sama secara bersamaan. Oleh karena itu, <strong>mengecek ketersediaan domain sejak awal adalah langkah paling krusial</strong> sebelum meluncurkan brand, usaha, atau lembaga Anda.
			</p>
		</div>
		...
		<div class="space-y-8">
			<div class="space-y-2">
				<div class="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Panduan Praktis</div>
				<h2 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">
			...
			<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
				{#each steps as step}
					<div class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/60 p-6 space-y-4 shadow-sm hover:border-emerald-500/40 transition-colors">
		...
		<div class="space-y-6">
			<div class="space-y-2">
				<div class="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Perbandingan Ekstensi</div>
				<h2 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">
			...
			<div class="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
				<table class="w-full text-left text-xs sm:text-sm text-slate-700 dark:text-slate-300">
					...
					<tbody class="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-900/40">
						<tr>
							<td class="p-4 font-bold text-emerald-600 dark:text-emerald-400">.COM</td>
							<td class="p-4">Bisnis Komersial, Toko Online, Umum</td>
							<td class="p-4"><span class="rounded bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-xs">Tanpa Syarat</span></td>
						...
						<tr>
							...
							<td class="p-4"><span class="rounded bg-amber-500/10 text-amber-700 dark:text-amber-400 px-2 py-0.5 text-xs">KTP + NIB/SIUP</span></td>
						...
						<tr>
							...
							<td class="p-4"><span class="rounded bg-amber-500/10 text-amber-700 dark:text-amber-400 px-2 py-0.5 text-xs">Surat Permohonan Sekolah</span></td>
						...
						<tr>
							...
							<td class="p-4"><span class="rounded bg-amber-500/10 text-amber-700 dark:text-amber-400 px-2 py-0.5 text-xs">SK Kemenag / Pengasuh</span></td>
		...
		<div class="rounded-3xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/50 p-8 space-y-6">
			<div class="flex items-center gap-3">
				...
				<div>
					<h3 class="text-xl font-bold text-slate-900 dark:text-white">Tips Memilih Nama Domain yang Bagus & Efektif</h3>
					<p class="text-xs text-slate-500 dark:text-slate-400">Rekomendasi praktis untuk daya saing SEO dan brand jangka panjang</p>
				</div>
			</div>
				...
				{#each domainTips as tip}
					<div class="space-y-2">
						<h4 class="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
							<CheckCircle2 class="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
							<span>{tip.title}</span>
						</h4>
		...
		<div class="rounded-3xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950/80 to-slate-900 p-8 text-white space-y-6">
			<div class="space-y-2">
				<h3 class="text-xl font-bold text-white">Bagaimana Jika Domain Sudah Ditemukan?</h3>
				<p class="text-xs sm:text-sm text-slate-300 leading-relaxed">
					Setelah memastikan nama domain impian Anda tersedia, langkah berikutnya adalah mendaftarkan domain beserta hosting dan paket pembuatan website. Barizaloka menyediakan layanan lengkap pendaftaran domain, setup SSL gratis, pembuatan website profesional, serta pendampingan kelola.
				</p>
			</div>
			...
			<div class="flex flex-wrap items-center gap-4 pt-2">
				<a
					...
					target="_blank"
					rel="noopener noreferrer"
					class="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-400 px-6 py-3.5 text-sm font-extrabold text-slate-950 shadow-lg shadow-emerald-500/25 hover:from-emerald-400 hover:to-teal-300 transition-all"
				>
				...
				<a
					href="/harga"
					class="inline-flex items-center gap-2 rounded-2xl border border-white/20 bg-white/10 px-6 py-3.5 text-sm font-bold text-white hover:bg-white/20 transition-all"
				>

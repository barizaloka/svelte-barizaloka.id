...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
	...
	<div class="grid grid-cols-1 gap-12 lg:grid-cols-12 items-center my-12">
		<div class="lg:col-span-7 space-y-6">
			...
			<h1 class="text-3xl sm:text-5xl font-black text-slate-900 dark:text-white leading-tight">
				Jasa Pembuatan Website di <span class="text-emerald-600 dark:text-emerald-400">{location.name}</span>
			</h1>
		...
		<div class="lg:col-span-5">
			<div class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/90 p-8 shadow-2xl space-y-6">
				<h3 class="text-lg font-bold text-slate-900 dark:text-white">Mengapa Pilih Barizaloka di {location.name}?</h3>
	...
	<div class="my-20 space-y-8">
		<div class="text-center max-w-2xl mx-auto space-y-2">
			<h2 class="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Spesialisasi Sektor di {location.name}</h2>
			<h3 class="text-2xl sm:text-4xl font-extrabold text-slate-900 dark:text-white">Pilih Layanan Niche di {location.name}</h3>
		</div>
	...
	{#if nearbyList.length > 0}
		<div class="my-16 rounded-3xl border border-slate-200 bg-slate-100/80 dark:border-slate-800 dark:bg-slate-900/40 p-8 text-center space-y-4">
			<h3 class="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Jasa Website di Wilayah Sekitar {location.name}</h3>
